...
---
Generated by Voice Context Manager | Enhanced Trello Voice OS
Card ID: ${context.id.split('_')[0] || 'unknown'}
Processing Date: ${context.timestamp.toLocaleDateString()}
`;
    }

    private async createAttachmentOnCard(cardId: string, context: VoiceContextEntry): Promise<boolean> {
        try {
            // Create the attachment filename with timestamp and context ID
            const filename = `voice_context_${context.timestamp.toISOString().split('T')[0]}_${context.id.slice(-8)}.txt`;
            
            // Create attachment content in markdown format
            const attachmentContent = this.generateAttachmentContent(context);
            
            // Convert content to data URL for Trello API
            const dataUrl = `data:text/plain;base64,${Buffer.from(attachmentContent).toString('base64')}`;
            
            // TODO: Use Trello MCP to create actual attachment
            // For now, this would need to be implemented with real MCP integration
            // Example call would be:
            // await attachImageToCard(cardId, dataUrl, filename);
            
            console.log(`📎 [READY] Would create attachment: ${filename} on card ${cardId}`);
            console.log(`📄 Content size: ${attachmentContent.length} characters`);
            
            // Return true to indicate successful attachment creation
            // In real implementation, this would depend on the MCP call result
            return true;
